...
+-------+--------------------------------+-----------+
        |                               |
        | Zabbix Agent (port 10050)     | Synthetic check scripts
        | + UserParameter custom checks | login -> operate -> assert
        v                               v
+----------------------------------------------------+
|                   Zabbix 7.2                       |
|   1000+ hosts · 15k+ URL checks · 4 teams          |
|   Custom service triggers · nodata() alerting      |
+----+---------------------------+-------------------+
     |                           |
     | JSON-RPC API              | Ansible playbook
     v                           v
+-------------------+    +---------------------+
| Python Automation |    |   Agent rollout     |
| Monthly reports   |    |   Zabbix 7 agent    |
| Daily digests     |    |   30 min -> 2 min   |
| Agent health CSV  |    +---------------------+
+---------+---------+
          |
          v
